(function () {
	...
	Colorpicker.prototype = {
		init(opt) {
			...
			// 自己写的生成到想要生成的div下面
			var body = document.getElementById("fontColourSlider"),
				div = document.createElement("div");
			...
			left = (window.innerWidth * 0.3 * 0.9 - 150) / 2;
			// console.log("left",left);
			// $("#fontColourSlider").children().eq(0).css("left",'"' + pickerLeft + '"')
			// 设置整个取色板最上层div
			util.css(div, {
				"position": "absolute",
				"z-index": 2,
			...
			// console.log("left",left,"top", top + this.bindElem.offsetHeight)
		...
		render: function () {
		// 	<div style='width:8em;padding:5px;background: #f9f9f9;display: flex;flex-flow: row wrap;align-content: space-around;justify-content: space-around;' class='color-palette'>
		// 	${this.getPaletteColorsItem()}
		// </div>
		// 229行切换按钮的  <svg viewBox="0 0 24 24" style="width: 24px; height: 24px; border: 1px solid transparent; border-radius: 5px;"><path fill="#333" d="M12,5.83L15.17,9L16.58,7.59L12,3L7.41,7.59L8.83,9L12,5.83Z"></path><path fill="#333" d="M12,18.17L8.83,15L7.42,16.41L12,21L16.59,16.41L15.17,15Z"></path></svg>
			//设定外层颜色div的宽度还有每个p元素的宽高
			// 修改右边宽度为原来的一半 
			var tpl =
				`
				<div style="position: inherit;z-index: 100;display: flex;box-shadow: rgba(0, 0, 0, 0.3) 0px 0px 2px, rgba(0, 0, 0, 0.3) 0px 4px 8px;">

					<div class="colorpicker-pancel" style="background: rgb(255, 255, 255);box-sizing: initial; width: 150px; font-family: Menlo;">
						<div style="width: 100%; padding-bottom: 55%; position: relative; border-radius: 2px 2px 0px 0px; overflow: hidden;">
							<div class="color-pancel" style="position: absolute; top: 0px; right: 0px; bottom: 0px; left: 0px; background: rgb(${this.rgba.r},${this.rgba.g},${this.rgba.b})">
								...
								<div class="saturation-white" style="position: absolute; top: 0px; right: 0px; bottom: 0px; left: 0px;">
									...
									<div class="pickerBtn" style="position: absolute; top: -6px; left: -6px; cursor: default;">
										<div style="width: 12px; height: 12px; border-radius: 6px; box-shadow: rgb(255, 255, 255) 0px 0px 0px 1px inset;">
										</div>
									</div>
								</div>
							</div>
						...
						<div style="padding: 0 6px;">
							<div class="flexbox-fix" style="display: flex;align-items: center;height: 40px;">
								...
								<div style="-webkit-box-flex: 1; flex: 1 1 0%;"><div style="height: 12px; position: relative;">
									<div style="position: absolute; top: 0px;right: 0px; bottom: 0px; left: 0px;">
										<div class="hue-horizontal" style="padding: 0px 0px; position: relative; height: 100%;">
											<style>
											...
											<div  class="colorBar-color-picker" style="position: absolute; left: -6px;">
												<div style="width: 12px; height: 12px; border-radius: 6px; transform: translate(-px, -1px); background-color: rgb(248, 248, 248); box-shadow: rgba(0, 0, 0, 0.37) 0px 1px 4px 0px;">
												</div>
											</div>
										</div>
									</div>
						...
						<div class="flexbox-fix" style="display: flex;">
							<div class="flexbox-fix colorpicker-inputWrap" style="">
									${this.getInputTpl()}
							</div>
		...
		// 下面这个span 位置在下面的 hex input框下面同级元素
		...
		// input 框去掉边框样式  box-shadow: rgb(218, 218, 218) 0px 0px 0px 1px inset;
		getInputTpl: function () {
			...
			switch (this.current_mode) {
				case 'hex':
					...
					current_mode_html += `
							<div style="padding-left: 28%; width: 100%;">
								<div style="position: relative;">
									<input readonly="readonly" class="colorpicker-hexInput" value="${hex}" spellcheck="false" style="font-size: 16px; color: rgb(51, 51, 51); width: 100%; border-radius: 2px; border: none; height: 21px; text-align: center;">
		...
		getPaletteColorsItem: function () {
			...
			palette.forEach(item => str += `<p style='width:0.8em;height:0.8em;background:${item};margin:0 2px;border: solid 1px #d0d0d0;'></p>`)
			return str;
		},
		setPosition(x, y) {
			...
			// this.pancel_width = this.elem_colorPancel.offsetWidth;
			// this.pancel_height = this.elem_colorPancel.offsetHeight;
			// x -= document.getElementsByClassName("color-pancel")[0].getBoundingClientRect().left;
			// y -= document.getElementsByClassName("color-pancel")[0].getBoundingClientRect().top;
